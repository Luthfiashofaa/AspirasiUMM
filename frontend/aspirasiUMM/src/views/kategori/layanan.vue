    ...
    <div class="container mt-5">
      ...
      >
        <div
          ...
          :key="complaint.id" style="margin-bottom: 15pt;"
        >
          <div
            class="card shadow-lg text-center p-4 custom-card d-flex flex-column justify-content-between"
            style="height: 100%"
          >
            ...
            <div class="text-center mb-3">
              ...
              />
              <h5 class="fw-bold">
                {{ complaint.user ? complaint.user.name : "User not found" }}
              </h5>
              <span
                class="badge px-3 py-2 text-white"
                :class="
                  complaint.status === 'answered' ? 'bg-primary' : 'bg-danger'
                "
              >
                {{ complaint.status }}
              </span>
            ...
            <router-link
              :to="{ name: 'comment', params: { id: complaint.id } }"
            >
              <button class="btn custom-btn-primary btn-block mt-3">
                Detail Pengaduan
              </button>
            </router-link>
          </div>
  ...
  computed: {
    filteredProfiles() {
      return this.profiles.filter((profiles) => {
        const matchesName = profiles.name
          .toLowerCase()
          .includes(this.searchQuery.toLowerCase());
        const matchesStatus =
          !this.selectedStatus || profiles.status === this.selectedStatus;
    ...
    filteredComplaints() {
      return this.complaints.filter((complaint) => {
        const matchesName = complaint.user
          ? complaint.user.name
              .toLowerCase()
              .includes(this.searchQuery.toLowerCase())
          : true;
        ...
        const matchesCategory = complaint.kategori.toLowerCase() === "layanan"; // Filter kategori Fasilitas
